...
const requiredId = users.find(
  (eachUser) =>
    eachUser.firstName === "Madison" && eachUser.lastName === "Marshall"
);
console.log(requiredId.id);

// 2) Who wrote the first comment (assuming the first comment is in position 0 of the comments array)
const firstCommenter = users.find(
  (eachUser) => eachUser.id === comments[0].userId
);
console.log(
  `The First Commenter is ${firstCommenter.firstName} ${firstCommenter.lastName}`
);

// 3) Which user commented 'OK great thanks'?
const commenterId = comments.find(
  (eachComment) => eachComment.text === "OK great thanks"
).userId;

const commenter = users.find((eachUser) => eachUser.id === commenterId);
console.log(`The Commenter is ${commenter.firstName} ${commenter.lastName}`);

// 4) Add the user's first and last name to each comment in the comments array
const newCommentsArray = comments.map((eachComment) => {
  const { firstName, lastName } = users.find(
    (eachUser) => eachUser.id === eachComment.userId
  );
  return {
    ...eachComment,
    firstName,
...
const notCommentedUsers = users.filter(
  (eachUser) =>
    !comments.some((eachComment) => eachComment.userId === eachUser.id)
);
console.log(notCommentedUsers);
